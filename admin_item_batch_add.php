...
<head>
    ...
    <title>SCU Maker物资管理系统 || 批量增加物资</title>
</head>
...
<body>
                ...
                <ul class="nav navbar-nav">
                    ...
                    <li class="active" class="dropdown">
                        ...
                        <ul class="dropdown-menu">
                            ...
                            <li class="active"><a href="admin_item_batch_add.php">批量增加物资</a></li>
    ...
    <h1 style="text-align: center"><strong>批量增加物资</strong></h1>
    <div style="padding: 10px 300px 10px;">
        <form action="admin_item_batch_add.php" method="POST" style="text-align: center" class="bs-example bs-example-form" role="form">
            <div id="item_batch_add">
                ...
                <div class="input-group"><span class="input-group-addon">添加</span><input name="nnum" type="text" type="text" placeholder="范围1-100" class="form-control"><span class="input-group-addon">个</span></div><br />
            </div>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        $nnam = replaceSpecialChar($_POST["nname"]);
        $nrmk = replaceSpecialChar($_POST["nremarks"]);
        $npri = (float)replaceSpecialChar($_POST["nprice"]);
        $nstkd = replaceSpecialChar($_POST["nstockdate"]);
        $ncla = replaceSpecialChar($_POST["nclassid"]);
        $nshelf = replaceSpecialChar($_POST["nshelfid"]);
        $num = (int)replaceSpecialChar($_POST['nnum']);
       

        if (is_integer($num) && $num > 0 && $num < 100 && isset($nnam) && isset($nrmk) && is_float($npri) && isset($nstkd) && isset($ncla) && isset($nshelf)) {
            ...
            if ($resa == 1) {
                ...
                echo "<script>window.location.href='admin_item_batch_add.php'</script>";
            }
        } else {
            // echo "<script>alert('输入有误，请重新输入！');</script>";
            echo "<script>alert('name={$nnam},rmk={$nrmk},price={$npri},date={$nstkd},class={$ncal},shelf={$nshelf},num={$num}')</script>";
            echo "<script>window.location.href='admin_item_batch_add.php'</script>";
        }
    }
